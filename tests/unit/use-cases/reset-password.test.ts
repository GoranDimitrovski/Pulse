...
describe('ResetPasswordUseCase', () => {
  ...
  async function issueResetToken(userId: string, expiresInMs = 60_000): Promise<string> {
  ...
  it('updates the password and revokes every refresh token for that user', async () => {
    ...
    const plainToken = await issueResetToken(user.id);
  ...
  it('rejects an expired reset token', async () => {
    const user = users.users[0]!;
    const plainToken = await issueResetToken(user.id, 60_000);
