...
/**
 * ponytail: logs the reset link instead of integrating a real SMTP/email provider.
...
export class ConsoleMailer implements IMailer {
  ...
  async sendPasswordReset(to: string, resetToken: string): Promise<void> {
    this.logger.info({ to, resetToken }, 'Password reset requested (email delivery not configured)');
